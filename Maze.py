...
class Maze:
    def __init__(self, dimensions):
        ...
        self.visited = list(list(0 for x in range(dimensions[0])) for y in range(dimensions[1]))    # visited matrix keeps track of which rooms have been visited
    ...
    def traverse(self, node):
        ...
        for x, y in [(0, 1), (-1, 0), (0, -1), (1, 0)]: # for all the adjacent rooms
            if 0 <= node[0] + 2 * x < len(self.maze) and 0 <= node[1] + 2 * y < len(self.maze[0]):
                if self.visited[(node[0] + 2 * x) // 2][(node[1] + 2 * y) // 2] == 0:   # if the room hasn't been visted yet
                    open.append((node[0] + 2 * x, node[1] + 2 * y))         # adds the room as traversable
                    self.visited[(node[0] + 2 * x) // 2][(node[1] + 2 * y) // 2] = 1    # sets the room as visited
        ...
        while len(open) > 0:    # takes a random room within the traversables and traverses it
            ...
            print(node, next_v, self.dimensions, (len(self.maze), len(self.maze[0])))
